...
import { publishTeamMember, archiveTeamMember } from "services";
...
const ViewMember = () => {
	...
	const publishMember = async () => {
		setLoading(true);
		await publish.mutateAsync({ catId: catId, id: id });
	};

	const archive = useMutation(archiveTeamMember, {
		onSuccess: () => {
			setIsOpen(false);
			setLoading(false);
		},
	...
	const archiveMember = async () => {
		setLoading(true);
		await archive.mutateAsync({ catId: catId, id: id });
	};
								...
								<div className="flex w-full mt-3 items-center justify-between">
									...
									<div className="flex">
										<button
											...
											disabled={member?.state === "published" ? true : false}>
											Publish
										</button>
										<button
											className={`${
											...
											disabled={member?.state === "archived" ? true : false}
											onClick={() => {
												setAction("archive");
												handleModal();
											}}>
