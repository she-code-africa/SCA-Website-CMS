...
const JobTypeCategory = () => {
	...
	const categories = useQuery("categories", getJobCategories);
	const types = useQuery("types", getJobTypes);

	useEffect(() => {
		if (categories.isSuccess) {
			setJobCategories(categories.data);
		}
		if (types.isSuccess) {
						...
						<div className="my-3">
							...
							<ul
								...
								{jobCategories?.map((jobCategory, index) => (
									<li key={index}>{jobCategory.name}</li>
								))}
							</ul>
						</div>
					</div>
